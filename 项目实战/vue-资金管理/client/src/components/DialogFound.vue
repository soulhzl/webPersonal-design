  ...
  <div class="logFund">
   ...
   <div class="form">
    ...
    <el-form-item prop='descripe' label="收支描述:">
      <el-input type="descripe" v-model="form.descripe"></el-input>
    </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      form_rules: {
        descripe: [{ required: true, message: "收支描述不能为空！", trigger: "blur" }],
    ...
    onSubmit(form) {
      this.$refs[form].validate(valid => {
        if (valid) {
          ...
          this.$axios.post(`/profile/${url}`, this.form).then(res => {
            const data = res.data
            if (data.code == 0) {
              this.$message({
                message: "保存成功！",
